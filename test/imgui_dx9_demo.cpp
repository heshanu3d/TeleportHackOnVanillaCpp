...
int main(int, char**)
{
    ...
    // ImFont* font = io.Fonts->AddFontFromFileTTF("D:\\code\\c++\\TeleportHackOnVanilla\\install\\Debug\\bin\\fonts\\ZhanKuWenYiTi-2.ttf", 18.0f, nullptr, io.Fonts->GetGlyphRangesChineseFull());
    //IM_ASSERT(font != nullptr);

    char path[MAX_PATH] = {0}; // 用于存储路径的数组
    if (GetModuleFileNameA(NULL, path, MAX_PATH)) { // 获取当前执行的模块的文件名
